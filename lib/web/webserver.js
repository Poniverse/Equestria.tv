...
function ipForRequest(req) {
    var ip = req.ip;
    if (Config.get('trusted-proxies').indexOf(ip) !== -1) {
...
function handleChannel(req, res) {
    if (!$util.isValidChannelName(req.params.channel)) {
        res.status(404);
        sendJade(res, "error", {
            message: "Invalid channel name '" + XSS.sanitizeText(req.params.channel) + "'"
        });
        return;
    }
    ...
    if (sio.charAt(sio.length - 1) == '/') {
        sio = sio.substr(0, sio.length - 1);
    }

    // This is just because we route socket.io requests
    // though our nginx server through the socket.io
    // directory
    if (sio.substr(sio.length - 9) == 'socket.io') {
        sio = sio.substr(0, sio.length - 10);
    }
...
function handleIndex(req, res) {
    db.channels.topX(20, function(err, rows){
        sendJade(res, "index", {
            channels: rows
        });
    });
            ...
            if (err) {
                if (err.message && err.message.match(/failed to decode param/i)) {
                    ...
                    sendJade(res, "error", {
                        ...
                        loginName: loginName,
                        message: "You seem to have stumbled upon a page that doesn't exist."
                    });
                    return;
