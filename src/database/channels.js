...
module.exports = {
    ...
    /**
     * Returns the given number of channels, sorted by user count.
     *
     * @param limit the number of channels to return
     * @param callback
     */
    topX: function (limit, callback) {
        if (typeof callback !== "function") {
            return;
        }

        db.query("SELECT * FROM `channels` WHERE `is_public` = 1 ORDER BY `user_count` DESC LIMIT ?",
            [limit],
            function (err, rows) {
                if (err) {
                    callback(err, null);
    ...
    /**
     * Saves "cached" information used to display channels on the homepage.
     *
     * @param channel the channel object to save data for
     * @param callback
     */
    saveCachedInfo: function (channel, callback) {
        if (typeof callback !== "function") {
            callback = blackHole;
        }

        if (!valid(channel.uniqueName)) {
            callback("Invalid channel name", null);
            return;
        }

        var currentTitle =
            channel.modules.playlist.current === null
            ? 'nothing right now!'
            : channel.modules.playlist.current.media.title;
        ...
        db.query(
            ...
            [
                ...
                channel.modules.options.opts.pagetitle,
                channel.modules.options.opts.show_public ? 1 : 0,
                channel.uniqueName
            ], callback);
    },
