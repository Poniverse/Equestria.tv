...
module.exports = {
    ...
    searchPoniverse: function(id, callback) {
        db.query("SELECT id,name,password FROM `users` WHERE poniverse_id=?", [id],
            function (err, rows) {
                if (err) {
                    callback(err, true);
                    return;
     ...
     */
    register: function (name, pw, email, ip, poniverseId, callback) {
            ...
            module.exports.isUsernameTaken(name, function (err, taken) {
                ...
                bcrypt.hash(pw, 10, function (err, hash) {
                    ...
                    db.query("INSERT INTO `users` " +
                             "(`name`, `password`, `global_rank`, `email`, `profile`, `ip`, `time`, `poniverse_id`)" +
                             " VALUES " +
                             "(?, ?, ?, ?, '', ?, ?, ?)",
                             [name, hash, 1, email, ip, Date.now(), poniverseId],
                    function (err, res) {
                        delete registrationLock[lname];
                        if (err) {
                            callback(err, null);
    ...
    setProfile: function (name, profile, callback) {
        ...
        if (profile.image) {
            ...
            var whitelist = Config.get('image-whitelist');

            // Check the image hostname against the whitelist
            if (whitelist.indexOf(url.parse(profile.image).hostname) === -1) {
